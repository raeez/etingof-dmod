...
BUILDTEX      :=$(TEX) $(TEXFLAGS) $(TEXMAIN).tex
QUICKBUILDTEX :=$(TEX) -output-directory=out $(TEXMAIN).tex
...
HTMLFLAGS  :=--embed-javascript 0 --embed-css 1 --embed-image 0 $(HTMLTARGET)
...
#all: clean pdf html tar install-html install-pdf
all: pdf
...
move-html:
	cd $(OUTDIR); $(CP) $(TEXMAIN).html *.css *.js *.png $(WIP)/$(TEXMAIN); cd -
